...
def parse_mnist(image_filename, label_filename):
    ...
    with gzip.open(image_filename, "rb") as f:
        X = np.frombuffer(f.read(), dtype=np.uint8, offset=16).reshape(-1, 784).astype(np.float32) / 255
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(step):
        ...
        X_batch = X[start:end]
        y_batch = y[start:end]
        index = np.arange(X_batch.shape[0])
        Z1 = X_batch @ W1
        Z1[Z1 < 0] = 0 # RELU
        G2 = softmax(Z1 @ W2)
        G2[index, y_batch] -= 1
        G1 = G2 @ W2.T
        G1[Z1 <= 0] = 0
        W1_grad = X_batch.T @ G1 / X_batch.shape[0]
        W2_grad = Z1.T @ G2 / X_batch.shape[0]
        W1 -= lr * W1_grad
        W2 -= lr * W2_grad
    ### END YOUR CODE
